        ...
        <section className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
          <Card className="border-zinc-200/80 bg-white/90 shadow-[0_20px_60px_-30px_rgba(24,24,27,0.35)]">
            <CardHeader>
              ...
              <CardDescription>
                Geschuetzte Arbeitsflaeche fuer Projekte, Tasks und Profilverwaltung.
              </CardDescription>
            </CardHeader>
            <CardContent className="grid gap-4 md:grid-cols-3">
              <div className="rounded-2xl border border-zinc-200 bg-zinc-50 p-4">
                <p className="text-sm text-zinc-500">Rolle</p>
                <p className="text-2xl font-semibold capitalize">{typedProfile.role}</p>
              </div>
              <div className="rounded-2xl border border-zinc-200 bg-zinc-50 p-4">
                <p className="text-sm text-zinc-500">Projekte</p>
                <p className="text-2xl font-semibold">{typedProjects.length}</p>
              </div>
              <div className="rounded-2xl border border-zinc-200 bg-zinc-50 p-4">
                <p className="text-sm text-zinc-500">Tasks</p>
                <p className="text-2xl font-semibold">{typedTasks.length}</p>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="border-zinc-200/80 bg-zinc-950 text-zinc-50 shadow-[0_20px_60px_-30px_rgba(24,24,27,0.55)]">
            ...
            <CardContent className="grid gap-3 text-sm">
              ...
              <div className="rounded-xl border border-white/10 bg-white/5 px-4 py-3">Weitere Nutzer starten als `member`.</div>
              <div className="rounded-xl border border-white/10 bg-white/5 px-4 py-3">RLS deckt `profiles`, `projects` und `tasks` ab.</div>
              <SignOutButton />
            </CardContent>
          </Card>
        </section>
        ...
        <section className="grid gap-6 lg:grid-cols-[0.8fr_1.2fr]">
          ...
          <Card className="border-zinc-200/80 bg-white/90">
            <CardHeader>
              ...
              <CardDescription>
                Serverseitig geladen aus Supabase. Aenderungen laufen ueber Server Actions.
              </CardDescription>
            </CardHeader>
            <CardContent>
              <ProjectBoard projects={typedProjects} tasks={typedTasks} />
            </CardContent>
          </Card>
        </section>
      </div>
